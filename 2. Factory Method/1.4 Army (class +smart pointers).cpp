...
public:
	...
	std::unique_ptr<Warrior> CreateWarrior(int num) {
		switch (num)
		{
		case 0: i++; return std::unique_ptr<Warrior>(new Infantryman); break;
		case 1: a++; return std::unique_ptr<Warrior>(new Archer);  break;
		case 2: h++; return std::unique_ptr<Warrior>(new Horseman);  break;
...
int main()
{
	srand(time(0));
	Factory ArmyFactory;
	std::vector<std::unique_ptr<Warrior> > Army;
